...
class CI_DB_oci8_driver extends CI_DB {
	...
	var $limit_used;
	var $bind = FALSE; //Added Bhupendra
...
//		return @ociplogon($this->username, $this->password, $this->pooled);
	}
	...
	//Orginal _execute function commented by Bhupendra
    function _execute($sql)
    {
        // oracle must parse the query before it is run. All of the actions with
        // the query are based on the statement id returned by ociparse
...
//        }
        ...
        if ($exec_worked === FALSE) { // if ociexecute failed, grab the oracle error message and log it
            ...
            log_message('error', $e['message']);
            print_r("<b>Error:</b>&nbsp;&nbsp;". $e['message']."<br /><b>SQL Query</b>:&nbsp;&nbsp;".$e['sqltext']);
        }
        return $exec_worked;
        //end modification
    }
		// start of vanitha addition
    ...
    function _stored_execute($sql,$params)
    {
        ...
        return $exec_worked;//@ociexecute($this->stmt_id, $this->_commit);
        //end modification
    }
	// end of vanitha addition
	/**
    ...
    // start of vanitha addtion
	function array_stored_procedure($package, $procedure, $params)
	{
    ...
    if ($package == '' OR $procedure == '' OR ! is_array($params))
        ...
        $this->binds = TRUE;
		//$this->_bind_params($params);
		$this->_stored_execute($sql,$params);
	}
    // end of vanitha addtion


	//Orignial stored procedure function commented by Bhupendra
	function stored_procedure($package, $procedure, $params)
	    {
        if ($package == '' OR $procedure == '' OR ! is_array($params))
        {
