...
jQuery(function(){
	...
	/* re-write drupal ajax prototype */
	if(typeof Drupal === 'object'){
		var def = Drupal.ajax.prototype.commands.insert;
		Drupal.ajax.prototype.commands.insert = function(){
			setTimeout(function(){
				initAjaxBlocksLoad();
				initLightbox();
			}, 100);
...
function initLoaderAnimation(){
	var blocks = jQuery('.mejs-overlay-loading');
	var i = 0;
	var cSpeed=15;
	var cTotalFrames=18;
	var cFrameWidth=68;
	...
	blocks.each(function(){
		var loaderBlock = jQuery(this);
		loaderBlock.attr('id', 'loaderImage' + i);
		i++;
...
function initSearchForm() {
	jQuery('.custom-search-form').submit(function(){
		var keyword = jQuery(this).find('.form-text').eq(0).val();
		location.href = jQuery(this).attr('action') + '/' + keyword;
		return false;
	});
}

function initCustomForms(){
	jQuery('form').not('#views-exposed-form-product-search-page').each(function(){
		jcf.customForms.replaceAll(this);
	});
	jQuery('form').not('#views-exposed-form-product-search-page').find('select').each(function(){
		var selectedOption = this.dropOpened;
		if(this.jcf && selectedOption){
			this.jcf.dropOpened = true;
			this.jcf.refreshState();
			this.jcf.dropOpened = false;
		}
	})
	jQuery("#edit-field-address-info-country-selective").change(function(event) {
		jQuery("#edit-field-address-info-locality-selective").val('All');
	});
}
...
function initCustomFilters(){
	...
	jQuery('.custom-filters').each(function(){
		...
		selectBoxes.each(function(){
			var resetBtn;
			var mainSelect = jQuery(this);
			var parent = mainSelect.closest('.select-box');
			
			/* duplicate multi select */
			jQuery('#views-exposed-form-product-search-page').find('select').each(function(){
				var origMulti = jQuery(this);
				if(mainSelect.attr('data-rel') == origMulti.attr('id')){
					resetBtn =jQuery('<a href="#" class="reset-all">x</a>').appendTo(parent);
					mainSelect.empty();
					origMulti.find('option').each(function(){
						if(jQuery(this).attr('selected') && window.location.href.indexOf(this.value) != -1){
							mainSelect.get(0).dropOpened = true;
						};
						mainSelect.append(jQuery(this).clone());
					});

					var defaultClone = mainSelect.clone();
					...
					resetBtn.bind('click', function(e){
						...
						jQuery(mainSelect[0].jcf.selectList).find('li.current-selected').removeClass('current-selected');
						mainSelect[0].jcf.destroy();
						
						//create and append default select
						var clone = defaultClone.clone().insertAfter(mainSelect);
						mainSelect.remove();
						mainSelect = clone.data('oldIndex', 0);
						if(selectOrig) {
							selectOrig = clone;
							mainSelect.data('selectOrig', selectOrig);
						}
			...
			var selectOrig = false;
			if(parent.length){
				selectOrig = mainSelect.clone();
				mainSelect.data('selectOrig', selectOrig);
				
				var addBox = parent.find('.add-box');
				var addButton = addBox.find('.add');
				...
				function removeFilter(){
					parent.find('.btn-remove').unbind('click.removeFilter').bind('click.removeFilter', function(){
						var mainForm = jQuery('#views-exposed-form-product-search-page');
						...
						form.find('select').each(function(i, select){
							jQuery(select).find('option').eq(removeIndex).removeAttr('disabled');
						});
						if(selectOrig){
							selectOrig.find('option').eq(removeIndex).removeAttr('disabled');
						}
						refreshCustomSelect();
						jQuery(this).closest('.new-filter').remove();
						if(jQuery('.ctools-auto-submit-click', jQuery('#views-exposed-form-product-search-page')).length) jQuery('.ctools-auto-submit-click', jQuery('#views-exposed-form-product-search-page')).click();
						return false;
					});
				}
			};
			...
			function reInitSelect(){
				jQuery('.custom-filters').find('select').each(function(){
					var select = jQuery(this);
					select.data('oldIndex', this.selectedIndex);
					select.unbind('change.Filters').bind('change.Filters', function(){
						var mainForm = jQuery('#views-exposed-form-product-search-page');
						var currentMultiSelect = mainForm.find('#' + jQuery(this).attr('data-rel'));
						if(currentMultiSelect){
							if(select.data('oldIndex') >= 0){
								currentMultiSelect.find('option').eq(select.data('oldIndex')).removeAttr('selected');
							}
							currentMultiSelect.find('option').eq(this.selectedIndex).attr('selected', 'selected');
							select.closest('.select-box').find('.reset-all').show();
						};
						
						if(select.data('selectOrig')){
							disabledOption(this);
						}
						select.data('oldIndex', this.selectedIndex);
						if(jQuery('.ctools-auto-submit-click', jQuery('#views-exposed-form-product-search-page')).length) jQuery('.ctools-auto-submit-click', jQuery('#views-exposed-form-product-search-page')).click();
			...
			function disabledOption(orig){
				jQuery(orig).closest('.select-box').find('select').not(orig).each(function(i, select){
					jQuery(select).find('option').eq(jQuery(orig).data('oldIndex')).removeAttr('disabled');
					jQuery(select).find('option').eq(orig.selectedIndex).attr('disabled', 'disabled');
				});
				jQuery(orig).data('selectOrig').find('option').eq(jQuery(orig).data('oldIndex')).removeAttr('disabled');
				jQuery(orig).data('selectOrig').find('option').eq(orig.selectedIndex).attr('disabled', 'disabled');
				jcf.customForms.replaceAll(self);
				refreshCustomSelect();
			};
			
			function refreshCustomSelect(init){
				form.find('select').each(function(i, select){
					if(select.jcf){
						jQuery(select.jcf.selectList).find('li').removeClass('disabled-state');
						jQuery(select).find('option').each(function(ind){
							if(jQuery(this).prop('disabled')){
								jQuery(select.jcf.selectList).find('li').eq(ind).addClass('disabled-state');
							}
						});
					};
				});
	...
	var selectColors = jQuery('#color-select');
	if(selectColors.length > 0) {
		var inputColors = jQuery('[id=' + selectColors.attr('data-rel') + ']');
		var options = selectColors.find('option');
		options.each(function(){
			var option = jQuery(this);
			var color = option.text();
			option.text('');
			option.data('color', color);
			jQuery('<span class="color-indicator"></span>').css({
				display: 'block',
				height: 10,
				marginRight:10,
				background: color
		...
		selectColors.change(function(){
			jQuery('[id=' + selectColors.attr('data-rel') + ']').val(options.eq(selectColors[0].selectedIndex).data('color'));
			if(jQuery('.ctools-auto-submit-click', jQuery('#views-exposed-form-product-search-page')).length) jQuery('.ctools-auto-submit-click', jQuery('#views-exposed-form-product-search-page')).click();
		})
	};
}
...
function initAjaxBlocksLoad(){
	...
	var minCountLinks = 3;
	var pagesSpacing = 1;
	jQuery('.list a.load-more').each(function(){
		if(jQuery(this).attr('data-rel')) {
			...
			var animSpeed = 500;
			if(container.length){
				var pageList = container.parent().find('.pager ul');
				currentIndex = pageList.find('.pager-current').length ? pageList.find('.pager-current').index() : 0;
				var pageLinks = pageList.find('a');
				var counterLinks = pageLinks.length - 1
				
				var pageItems = pageList.find('li');
				
				if(currentIndex >= pageItems.length -1) opener.hide();
				if(pageItems.filter('.' + activeClass).length > 0) currentIndex = pageItems.filter('.' + activeClass).index();
				pageItems.each(function(){
					jQuery(this).data('num', jQuery(this).index());
				});
				...
				opener.unbind('click').bind('click', function(e){
					...
					if(!ajaxBusy && currentIndex < pageLinks.length) {
						...
						jQuery.ajax({
							url: pageLinks.eq(currentIndex).attr('href'),
							type:'get',
							dataType:'text',
							success:function(msg) {
								pageLinks.eq(currentIndex).parent().html(pageLinks.eq(currentIndex).parent().text());
								numFlag++;
								var box = jQuery('<div />').html(msg);
								var newProjects = box.find('.projects .project');
								var loadedPopup = jQuery('<div id=page' + numFlag + '/>').html(newProjects).hide().appendTo(container).fadeIn();
								currentIndex++;
								ajaxBusy = false;
								reformat();
								...
								//reinit functions for new content
								initAnchorScroll(newProjects);
								initLightbox(newProjects);
							},
							error:function() {
								alert('AJAX Error!');
							}
				...
				pageLinks.on('click', function(){
					jQuery(document).ajaxStop(function(){
						var newContainer = jQuery('#' + opener.attr('data-rel')).parent();
						initAnchorScroll(newContainer);
						initLightbox(newContainer);
					});
				});
				
				// pagination events on click
				// pageItems.bind('click', function(e){
					// e.preventDefault();
					// var link = jQuery(this);
					// if(jQuery('#page' + (link.data('num')+1)).length > 0){
						// jQuery('html, body').animate({scrollTop: jQuery('#page' + (link.index()+1)).offset().top}, animSpeed)
					// }
					// else if(link.data('num') === (currentIndex+1) && jQuery('#page' + (link.data('num')+1)).length === 0) {
						// var anchorId = ('#page' + (link.data('num')+1));
						// opener.trigger('click');
						// setTimeout(function(){
							// jQuery('html, body').animate({scrollTop: jQuery(anchorId).offset().top}, animSpeed)
						// }, animSpeed)
					// }
					// else if(link.data('num') === counterLinks && jQuery('#page' + (link.data('num')+1)).length === 0) {
						// jQuery('html, body').animate({scrollTop: opener.offset().top - 400}, animSpeed)
					// }
					// else if(link.index() === 0){
						// jQuery('html, body').animate({scrollTop: container.offset().top}, animSpeed)
					// }
				// });
				
				// set dots in pagination
				function reformat() {
					pageList.find('.more-pages').remove();
					pageItems.removeClass(classHidden);
					pageItems.eq(currentIndex).addClass(activeClass).addClass('pager-current');
		
					var activeItems = pageList.find('li').filter('.active');
					
					var firstActive = activeItems.eq(0).index();
					var lastActive = activeItems.eq(activeItems.length-1).index();
					if(firstActive > pagesSpacing+1){
						jQuery('<li class="more-pages">...</li>').insertBefore(pageItems.eq(firstActive - pagesSpacing))
					};
					if(lastActive + pagesSpacing + 1 < pageItems.length - 1){
						jQuery('<li class="more-pages">...</li>').insertAfter(pageItems.eq(lastActive + pagesSpacing))
					};
					
					pageItems.each(function(hiddenIndex){
						if(
							(hiddenIndex > 0 && hiddenIndex < (firstActive - pagesSpacing))
							|| (hiddenIndex > (lastActive + pagesSpacing ) && hiddenIndex < (pageItems.length-1))
						){
							pageItems.eq(hiddenIndex).addClass(classHidden);
						}
					});
				};
				reformat();
	...
	//history load
	jQuery('.history-block a.load-more').each(function(){
		if(jQuery(this).attr('data-rel')) {
			var opener = jQuery(this);
			var src = opener.attr('href');
			var container = jQuery('#' + opener.attr('data-rel'));
			...
			if(container.length > 0){
				var switcherLinks = container.parent().find('.year-links a');
				opener.bind('click', function(e){
					if(src !== '#'){
						e.preventDefault();
						if(!ajaxBusy && startIndex < jQuery('#' + opener.attr('data-rel')).find('.pager a').length) {
							// load external content
							ajaxBusy = true;
							container.addClass(loadingClass);
							ajaxBusy = true;
							jQuery.ajax({
								url: jQuery('#' + opener.attr('data-rel')).find('.pager a').eq(startIndex).attr('href'),
								type:'get',
								dataType:'text',
								success:function(msg) {
									startIndex++;
									var box = jQuery('<div/>').html(msg);
									box.find('.info-block').each(function(){
										var item = jQuery(this);
										var idBox = item.attr('id');
										var loadedPopup = item;
										if(container.find('#' + idBox).length){
											loadedPopup = item.find('.box');
											container.find('#' + idBox).find('.info-content').append(loadedPopup.fadeIn());
										}
										else {
											container.find('.view-content').eq(0).append(loadedPopup.fadeIn());
										};
										evenBlocks(loadedPopup.find('.info-block'));
									});
									
									ajaxBusy = false;
									if(startIndex > jQuery('#' + opener.attr('data-rel')).find('.pager a').length - 1) opener.hide();
				...
				function evenBlocks(boxHolder){
					if(ie8) jQuery('.info-content > .box', boxHolder).filter(':nth-child(2n)').addClass('even');
				}
				evenBlocks(container.find('.info-block'));
				
				/* Years anchor */
				function refreshAnchorLink(){
					jQuery('.region-years .year-links a').each(function(){
						var link = jQuery(this);
						var href = link.attr('href');
						if(href != '#' && jQuery(href).length){
							jQuery(href).addClass('loaded');
							link.unbind('click').bind('click', function(e){
								jQuery('html, body').animate({scrollTop: jQuery(href).offset().top}, animSpeed);
								return false;
							})
						}
					});
...
function initMediaElement(box) {
	if(!box) var box = jQuery('.carousel');
	jQuery('video', box).each(function(){
		var video = jQuery(this).attr('poster', '');
		var slide = video.parents('.slide').data('videoLength', true);
		var w = slide.width();
		
		video.mediaelementplayer({
			videoHeight: w*9/21,
			videoWidth: w,
			pluginPath: 'http://sedaremirates.com/sites/all/themes/sedar/js/',
			...
			success: function(mediaElement, domObject){
				...
				slide.data('video', player);
				slide.css({
					height: this.videoHeight
				});
				mediaElement.setVideoSize(w, w*9/16);
				mediaElement.addEventListener('pause', function(e) {
					var parent = slide.not('.active');
					parent.find('.mejs-poster').show();
				}, false);
				mediaElement.addEventListener('play', function(e) {
					var parent = slide.not('.active');
					parent.find('.mejs-poster').hide();
				}, false);
			}
		});
	})
...
function initCycleCarousel() {
	jQuery('div.carousel').cycleGallery({
		animSpeed: 500,
		prevBtn: 'a.btn-prev',
		nextBtn: 'a.btn-next',
		generateSwitcherTo: '.switcher',
		onInit: function(obj){
			initMediaElement(obj);
			initLoaderAnimation();
			setTimeout(function(){
				var activeSlide = obj.data('cycleGallery').activeSlide;
				obj.data('cycleGallery').slides.removeClass('active');
				activeSlide.addClass('active');
				
				function startVideo(){
					if(activeSlide.data('video')) activeSlide.data('video').video.play();
					else {
						setTimeout(startVideo, 50);
					}
				}
				if(activeSlide.data('videoLength')){
					startVideo();
				};
			}, 20);
		},
		onBeforeChange: function(obj){
			obj.data('cycleGallery').slides.removeClass('active');
			obj.data('cycleGallery').slides.each(function(){
				var slide = jQuery(this);
				if(slide.data('video')){
					slide.data('video').video.stop();
				}
			})
		},
		onAfterChange: function(obj){
			var maskOffset = obj.data('cycleGallery').mask.offset().left;
			obj.data('cycleGallery').slides.each(function(){
				var slide = jQuery(this);
				if(slide.offset().left === maskOffset && slide.data('video')){
					var data = obj.data('cycleGallery');
					data = jQuery.extend({
						activeSlide: slide
					}, data);
					slide.addClass('active');
					slide.data('video').video.play();
				}
			})
...
function initLightbox(box) {
	if(!box) var box = jQuery('body');
	var fullMode = false;
	jQuery('.lightbox', box).each(function(){
		...
		link.fancybox({
			...
			onComplete: function(box) {
				if(link.attr('href').indexOf('#') === -1) {
					var fancy = jQuery('#fancybox-content');
					fancy.find('a.close').unbind('click.fb').bind('click.fb', function(e){
						jQuery.fancybox.close();
						e.preventDefault();
					});
					initLightbox(fancy);
					initSlideShow(fancy);
					initAnchorScroll(fancy);
					stButtons.locateElements();
					if(fancy.find('div.full').length > 0){
						jQuery('html').css({overflow: 'hidden'});
						fancy.css({overflow: 'visible'});
						stButtons.locateElements();
						initSlideShow(fancy);
						jcf.customForms.replaceAll();
						initTabs(fancy);
			...
			onClosed: function(){
				if(fullMode) {
					jQuery('html').add(jQuery('#fancybox-content')).removeAttr('style');
					fullMode = false;
				}
			}
		});
	});
	jQuery('.open-lightbox').each(function(){
		var link = jQuery(this);
		link.fancybox({
			padding: 0,
			overlayShow: true,
	...
	jQuery('.projects .project a.save, .product-detail a.save').each(function(){
		var link = jQuery(this);
		link.fancybox({
			padding: 0,
			overlayShow: true,
			...
			onComplete: function(box) {
				if(link.attr('href').indexOf('#') === 0) {
					jQuery('#fancybox-content').find('a.btn2').unbind('click.fb').bind('click.fb', function(e){
						jQuery.fancybox.close();
						e.preventDefault();
					});
				}
				jQuery(box).closest('.project').addClass('added');
			}
		});
	});
	jQuery('.projects .project a.remove, .product-detail a.remove').each(function(){
		var link = jQuery(this);
		link.fancybox({
			padding: 0,
			overlayShow: true,
			...
			onComplete: function(box) {
				if(link.attr('href').indexOf('#') === 0) {
					...
					jQuery('#fancybox-content').find('a.btn').unbind('click.fb').bind('click.fb', function(e){
						jQuery(box).closest('.project').removeClass('added');
						jQuery.fancybox.close();
						e.preventDefault();
					});
				}
...
function initSlideShow(box) {
	...
	jQuery('.full', box).fadeGallery({
		slides: '.images > img',
		btnPrev: 'a.prev',
		btnNext: 'a.next',
		pagerLinks: '.photos li',
		event: 'click',
		useSwipe: true,
		autoRotation: false,
		autoHeight: true,
	...
	jQuery('.product-view').fadeGallery({
		slides: '.image-holder > *',
		pagerLinks: '.photo-gallery li',
		event: 'click',
		useSwipe: true,
		autoRotation: false,
		autoHeight: true,
...
function infiniteScroll(){
	jQuery('.block-views .pager-next > a').each(function(){
		var opener = jQuery(this);
		var body = jQuery('body');
		var win = jQuery(window);
		var container = opener.closest('.view').find('.view-content > *');
		var bottomPos = container.offset().top + container.outerHeight();
		var loadingClass = 'ajax-load';
		var ajaxBusy, loadedContent;
		opener.unbind('click').bind('click', function(e){
			e.preventDefault();
			var src = opener.attr('href');
			if(src !== '#'){
					...
					jQuery.ajax({
						...
						success:function(msg) {
							if(msg) {
								...
								var loadedContent = jQuery('<div/>').html(newContent.find('.infinite-box > *')).hide().appendTo(container).fadeIn(function(){
									//init lightbox call in new content
									initLightbox(loadedContent);
									loadedContent.find('>*:eq(0)').unwrap();
								});
								if(newContent.find('.pager-next > a').length > 0) {
									opener.attr('href', newContent.find('.pager-next > a').attr('href'));
									bottomPos += loadedContent.outerHeight();
									ajaxBusy = false;
								}
								else {
									ajaxBusy = true;
		...
		win.bind('scroll load', function(){
			if(win.scrollTop() + win.height() >= (bottomPos - 40) && opener.length > 0){
				opener.trigger('click');
			}
		})
	});
...
function initTabs(box) {
	if(!box) var box = jQuery('body');
	jQuery('.tabs .tabset', box).contentTabs({
		autoHeight: true,
		tabLinks: 'a',
		onInit: function(obj){
			//amimate effect function
			var tabLinks = obj.find('a');
			var animSpeed = 600;
			if(jQuery.browser.msie && jQuery.browser.version < 10)  var noCssEffect = true;
			tabLinks.each(function(){
				var tabLink = jQuery(this);
				setTimeout(function(){
					var tabContent = tabLink.data().ctab;
					if(tabContent.hasClass('js-tab-hidden')){
						if(noCssEffect){
							tabContent.find('.sort-list li').css({opacity: 0});
						}
						else {
							tabContent.find('.sort-list li').addClass('hidden-item');
						}
					};
				},10);
			});
			tabLinks.bind('click', function(){
				...
				if(!tabLink.hasClass('active')){
					tabLinks.not(tabLink).each(function(){
						if (noCssEffect) {
							tabLink.data().ctab.find('.sort-list li').stop().animate({opacity: 1},animSpeed);
							jQuery(this).data().ctab.find('.sort-list li').not('.hidden-item').css({opacity: 0});
						}
						else {
							tabLink.data().ctab.find('.sort-list li').removeClass('hidden-item');
							jQuery(this).data().ctab.find('.sort-list li').not('.hidden-item').addClass('hidden-item');
						}
					})
				}
			})
	...
	jQuery('.colors', box).contentTabs({
		...
		onInit: function(obj){
			...
			obj.find('a').unbind('click').bind('click', function(){
				...
				if(!tab.find('li:first-child').hasClass('active')){
					tab.find('li:first-child').trigger('click');
				}
			})
		}
	});
...
;(function($){
	$.fn.contentTabs = function(o){
		...
		return this.each(function(){
			...
			function makeCallback(name, obj) {
...
jQuery.fn.cycleGallery = function(opt) {
	var options = jQuery.extend({
		...
		prevBtn: 'a.btn-prev',
		nextBtn: 'a.btn-next',
		switcher: '.pagination li',
		autorotation: false,
		switchTime: 1000,
		generateSwitcherTo: '.pagination',
		step: 1,
		startElement: 1,
		activeClass: 'active'
	}, opt);
	...
	return this.each(function(){
		var holder = jQuery(this),
			mask = holder.find(options.mask),
			slideset = mask.find(options.slideset).css({
				'position': 'absolute',
				width: 'auto',
				margin: 0,
				left: 0,
			...
			slider = mask.find('.slider').css({position: 'relative'}),
			slides = slideset.find(options.slides),
			slideWidth = slides.outerWidth(true),
			slidesetWidth = slides.length * slideWidth,
			btnPrev = holder.find(options.prevBtn),
			btnNext = holder.find(options.nextBtn),
			switcherItems = holder.find(options.switcher).removeClass(options.activeClass),
			currentElement = options.startElement, 
			step = options.step,
			animSpeed = options.animSpeed,
			isAnimating, timer, direction,
			currentPosition = (currentElement-1) * slideWidth;
		if(switcherItems.length == 0) {
			var switcher = jQuery('<ul></ul>').appendTo(options.generateSwitcherTo, holder);
			for(var i = 0; i < slides.length; i++){
				jQuery('<li><a href="#"></a></li>').appendTo(switcher);
			}
			switcherItems = switcher.find('li');
		}
		var activeIndex = switcherItems.eq(currentPosition / slideWidth).index();
		switcherItems.eq(activeIndex).addClass(options.activeClass);
		slider.css({
		...
		if(slides.length == 1) {
			btnNext.add(btnPrev).add(switcherItems).hide();
			holder.addClass('one-slide');
			if(slides.find('video').length) holder.addClass('video-slide');
		}
		
		//creating side clones of gallery
		slideset.clone().insertAfter(slideset).css({left: slidesetWidth}).clone().insertBefore(slideset).css({left: -slidesetWidth}).clone().insertBefore(slideset).css({left: -slidesetWidth*2});
		var cycleGallery = {
			mask: mask,
			slider: slider,
			slides: mask.find(options.slides),
			btnPrev: btnPrev,
			btnNext: btnNext,
			switcherItems: switcherItems,
			activeSlide: slides.eq(activeIndex),
			clonedSlidesets: slideset.siblings(),
		};
		holder.data('cycleGallery', cycleGallery);
		holder.addClass('js-ready');
		makeCallback('onInit', holder);
		autoRotation();
		...
		btnPrev.on('click', function(e){
			e.preventDefault();
			direction = 1;
			(activeIndex - 1) >= 0 ? activeIndex-- : activeIndex = slides.length - 1;
			switchSlide(step);
		});
		btnNext.on('click', function(e){
			e.preventDefault();
			direction = -1;
			(activeIndex + 1) < slides.length ? activeIndex++ : activeIndex = 0;
			switchSlide(step);
		});
		switcherItems.bind('click', function(e){
			e.preventDefault();
			var nextIndex = jQuery(this).index();
			var num = Math.abs(nextIndex - activeIndex);
			nextIndex > activeIndex ? direction = -1 : direction = 1;
			activeIndex = nextIndex;
			switchSlide(num);
		})

		function switchSlide(num){
			makeCallback('onBeforeChange', holder);
			if (!isAnimating) {
				isAnimating = true;
				if (currentPosition >= slidesetWidth || currentPosition <= -slidesetWidth) {
					currentPosition = 0
					slider.css({left: 0})
				}
				currentPosition = currentPosition + slideWidth * num * direction;
				slider.stop().animate({
					left: currentPosition
				}, animSpeed, function(){
					isAnimating = false
				...
				switcherItems.removeClass(options.activeClass).eq(activeIndex).addClass(options.activeClass);
				setTimeout(function(){
					makeCallback('onAfterChange', holder);
				}, animSpeed+50)
			}
		}
		...
		function autoRotation(){
			if (options.autorotation) {
				clearInterval(timer);
				direction = -1;
				timer = setInterval(switchSlide, options.switchTime);
			}
		}
		function makeCallback(name, obj) {
			if(typeof options[name] === 'function') {
...
;(function($) {
	...
	ContentPopup.prototype = {
		...
		outsideClickHandler: function(e) {
			...
			if(!$(currentNode).parents().filter(this.holder).length && !$(currentNode).parents().filter('#fancybox-content').length) {
				this.hidePopup();
			}
		},
		showPopup: function() {
...
jcf.addModule({
	...
	onOptionClick: function(e){
		...
		if(opener && !jcf.lib.hasClass(opener, 'disabled-state')) {
			this.dropOpened = true;
...
PlatformDetect.addRule({type: 'android', version: 2, css: 'android.css'});
PlatformDetect.addRule({type: 'android', version: 4, css: 'android4.css'});
